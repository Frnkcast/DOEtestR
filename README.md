...
1.  La interfaz genera una serie de carpetas de trabajo, con una raiz en
    la ubicación del proyecto actual. Se puede cambiar esta carpeta
    usando el paquete ‘here’.
2.  DOEtestR trabaja generando tablas relacionales, donde cada tema
    produce 2 tipos de tablas: Sets de datos, y Sets de problemas. A
    partir de la selección de datos, problemas y temas, doeExam() crea
    un examen. Para mantener un orden interno de las tablas generadas,
    se tiene una base de datos maestra denominada “Directorio”.
3.  Si se esta iniciando la construccion de un examen por primera vez,
    doeExam() llama a funciones internas para poblar el Directorio con
    tablas de problemas. Se puede elegir el número de sets de datos y el
    numero de problemas a generar por tema, para tener un banco de
    preguntas tan extenso como se desee. Si ya existen las tablas
    requeridas, se usan estas a modo de banco de preguntas.
4.  Una vez seleccionados los problemas, se ensamblan los enunciados de
    un examen, y se genera un archivo de texto con los enunciados en el
    formato especificado (LaTeX o HTML).
    1.  NOTA!! doeExam() solo ensambla los enunciados con el formato
        deseado, más no tiene un template para el examen en general. Sin
        embargo, basta con extraer los enunciados del archivo de texto
        para usarse.
5.  Adicionalmente, doeExam() y doePractice() generan un archivo de
    reporte, con las soluciones a los problemas seleccionados para el
    examen.
